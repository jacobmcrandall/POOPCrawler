    ...
    public class GameLayer : CCTileMap
    {
        ...
        public GameLayer() : base(/*"level_" + CCRandom.GetRandomInt(1, numLevels) + ".tmx"*/"level_2.tmx") // get a random level and load it on initialization
        {
            ...
            userInfo = new CCLabel
                ("Health: " + user.health + "/" + user.maxHealth + "    Attack : " + user.weapon.attack,"arial",6);
            userInfo.Position = new CCPoint(VisibleBoundsWorldspace.UpperRight.X + 20, VisibleBoundsWorldspace.UpperRight.Y + 5);
        ...
        void RunGameLogic(float frameTimeInSeconds)
        {
            ...
            foreach (character enemy in enemiesList)
            {
                ...
                {
                    if (user.attacked(enemy.weapon.attack))
                        userDeath();
                }
        ...
        void tileHandler(CCPoint world, CCTileMapLayer layer, Boolean calledByLoop=false)
        {
            ...
            if (!calledByLoop && properties != null && properties.ContainsKey("walkable") && properties["walkable"] == "true")
            {
                ...
                if(isTileOccupied(tileAtXy) && isUserNear(tileAtXy))
                {
                    ...
                    if (enemy.attacked(user.weapon.attack))
                        enemyDeath(enemy);
                }
